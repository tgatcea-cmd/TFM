...
final class FlutterBluePlusLinux extends FlutterBluePlusPlatform {
  ...
  BmScanResponse _mapDeviceToScanResponse(BlueZDevice device) {
  ...
  Stream<BmScanResponse> get onScanResponse {
    final deviceAddedStream = _client.deviceAdded.map((device) {
      return _mapDeviceToScanResponse(device);
    });

    final propertiesChangedStream = _client.devicesChanged.switchMap(
      (devices) {
        return mergeStreams(
          devices.map(
            (device) {
              return device.propertiesChanged.where((properties) {
                return properties.any(
                  (p) => const {'RSSI', 'Name', 'ManufacturerData', 'ServiceData', 'UUIDs'}.contains(p),
                );
              }).map((_) => _mapDeviceToScanResponse(device));
            },
          ),
        );
      },
    ...
    return mergeStreams([deviceAddedStream, propertiesChangedStream]);
  }
