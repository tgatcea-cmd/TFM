...
class InferenceBridge {
  ...
  Future<void> runIrrigationRecommendation([String? deviceId]) async {
    ...
    print(
      '[LocalDB Inference Verbose] Final Verdict: Verdict: ${result['verdict']}',
    );
  ...
  Map<String, dynamic> evaluateRecommendation({
    ...
    print(
      '$verbosePrefix Raw predHum Input: $predHum | Normalized: $normalizedPredHum',
    );
    ...
    if (activeModel != null) {
      print(
        '$verbosePrefix Using dynamic JSON model: ${activeModel.cropName} (${activeModel.modelId})',
      );
      try {
        final rfJson = jsonDecode(activeModel.treeDataJson);
        final dynamicRf = DynamicRandomForest.fromJson(rfJson);
        probs = dynamicRf.predict([radSum, normalizedPredHum]);
      } catch (e) {
        print(
          '$verbosePrefix ERROR Parsing dynamic model: $e. Falling back to legacy.',
        );
        probs = rf.score([radSum, normalizedPredHum]);
      }
    } else {
      print(
        '$verbosePrefix Notice: No active dynamic model selected. Using hardcoded legacy RF.',
      );
      probs = rf.score([radSum, normalizedPredHum]);
    }
    // ----------------------------------------
    ...
    print(
      '$verbosePrefix RF Model Score Input: [$radSum, $normalizedPredHum] -> Output Probs: $probs',
    );
